...
class DocumentChildHandler
{
    ...
    public function handle($parentModel, $documentType, array $validated)
    {
        ...
        if (count($classNames) !== count($relationNames)) {
            throw new \Exception(
                "Le nombre de classes et de relations ne correspond pas !"
            );
        }
        ...
        foreach ($classNames as $index => $className) {
            ...
            if (!method_exists($currentParent, $relationName)) {
                throw new \Exception(
                    "Relation '{$relationName}' introuvable sur " .
                        get_class($currentParent)
                );
            }
            ...
            if (isset($relations)) {
                foreach ($relations as $currentRelationName => $relationData) {
                    if (!method_exists($child, $currentRelationName)) {
                        throw new \Exception(
                            "Relation {$currentRelationName} introuvable sur " .
                                get_class($child)
                        );
                    }
                    ...
                    if (array_is_list($relationData) === false) {
                        ...
                        $relationModel = $this->fillModelAttributesRelation(
                            $relationModel,
                            $relationData
                        );
    ...
    function OldfillModelAttributes($model, array $data)
    {
        ...
        foreach ($data as $key => $value) {
            if (in_array($key, $model->getFillable())) {
                ...
                throw new \Exception(
                    "La clé '{$key}' n'existe pas dans les champs fillable de " .
                        get_class($model)
                );
            }
        }
    ...
    private function mapData(
        ...
        $map = [
            "facture-fournisseur-medical" => [
                ...
                "provider_reference" => fn($v) => $v["reference_fournisseur"] ??
                    null,
                "deposit_date" => fn($v) => isset($v["dateDepot"])
                    ? Carbon::parse($v["dateDepot"])->format("Y-m-d H:i:s")
                    : null,
            ],
            ...
            "mission" => [
                "data" => [
                    ...
                    "departure_date_base_planned" => fn($v) => $this->toDate(
                        $v["departure_date_base_planned"] ?? null
                    ),

                    "departure_time_base_planned" => fn($v) => $v[
                        "departure_time_base_planned"
                    ] ?? null,

                    "arrival_date_base_planned" => fn($v) => $this->toDate(
                        $v["arrival_date_base_planned"] ?? null
                    ),

                    "arrival_time_base_planned" => fn($v) => $v[
                    ...
                    "departure_date_base_actual" => fn($v) => $this->toDate(
                        $v["departure_date_base_actual"] ?? null
                    ),

                    "departure_time_base_actual" => fn($v) => $v[
                        "departure_time_base_actual"
                    ] ?? null,

                    "arrival_date_base_actual" => fn($v) => $this->toDate(
                        $v["arrival_date_base_actual"] ?? null
                    ),

                    "arrival_time_base_actual" => fn($v) => $v[
                    ...
                    "departure_date_site_planned" => fn($v) => $this->toDate(
                        $v["departure_date_site_planned"] ?? null
                    ),

                    "departure_time_site_planned" => fn($v) => $v[
                        "departure_time_site_planned"
                    ] ?? null,

                    "arrival_date_site_planned" => fn($v) => $this->toDate(
                        $v["arrival_date_site_planned"] ?? null
                    ),

                    "arrival_time_site_planned" => fn($v) => $v[
                    ...
                    "departure_date_site_actual" => fn($v) => $this->toDate(
                        $v["departure_date_site_actual"] ?? null
                    ),

                    "departure_time_site_actual" => fn($v) => $v[
                        "departure_time_site_actual"
                    ] ?? null,

                    "arrival_date_site_actual" => fn($v) => $this->toDate(
                        $v["arrival_date_site_actual"] ?? null
                    ),

                    "arrival_time_site_actual" => fn($v) => $v[
        ...
        foreach ($dataLooper as $field => $callback) {
            if (!is_callable($callback)) {
                throw new \Exception(
                    "Callback invalide pour le champ: {$field}"
                );
            }
        ...
        foreach ($relationLooper as $relation => $callback) {
            if (!is_callable($callback)) {
                throw new \Exception(
                    "Relation callback invalide pour: {$relation}"
                );
            }
