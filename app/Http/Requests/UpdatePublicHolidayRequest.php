...
class UpdatePublicHolidayRequest extends FormRequest
{
    ...
    {
        $publicHoliday = $this->route('publicHoliday');

        $publicHolidayId = $publicHoliday instanceof PublicHoliday
            ? $publicHoliday->id
            : $publicHoliday;
            ...
            'date' => [
                ...
                Rule::unique('public_holidays', 'date')
                    ->where(function ($query) {
                        return $query->where(
                            'work_calendar_id',
                            $this->input('work_calendar_id')
                        );
                    })
                    ->ignore($publicHolidayId),
            ],
    ...
    public function messages(): array
    {
        return [
            ...
            'date.unique' =>
                'Un jour férié existe déjà à cette date pour ce calendrier.',
