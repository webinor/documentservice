...
class FolderController extends Controller
{
    ...
    public function getFoldersWithPermissions(Request $request)
    {
        ...
        if ($folderId) {
            ...
            $department_folders = DepartmentFolder::whereHas(
                "folder.parent",
                function ($query) use ($current_folder) {
            ...
            $documents = Document::with([
                ...
                ->whereFolderId($folderId)
                ->get();

            //throw new Exception(json_encode($documents), 1);
        } else {
            $department_folders = DepartmentFolder::doesntHave("folder.parent")
                ->with("folder")
                ->whereDepartmentId($departmentId)
                ->get();
        }
        ...
        if (!$documents->isEmpty()) {
            ...
            $documentsWithPermissions = $this->filterDocuments(
                $documents,
                $documentTypesWithPermissions
            );
        }

        $foldersAndDocuments = $this->mergeFoldersAndDocuments(
            $foldersWithPermissions,
            $documentsWithPermissions
        );
        ...
        return response()->json([
            "success" => true,
            "pathSegments" => $current_folder
                ? $current_folder->getPathSegments()
                : [],
        ...
        return response()->json(
            [
                ...
                "document_type" => $documentTypesWithPermissions,
            ],
            200
        );
    }
    public function mergeFoldersAndDocuments(
        $foldersWithPermissions,
        $documentsWithPermissions
    ) {
        $items = collect([]);
        ...
        foreach ($documentsWithPermissions as $document) {
            $attachmentType =
                $document["main_attachment"]["file"]["type"] ?? null;
            $attachmentIcon = "📄";
            ...
            $items->push([
                ...
                "view_url" =>
                    ...
                    "/details-du-document/{$document["id"]}",
    ...
    public function filterDocuments($documentsList, $documentTypesList)
    {
        ...
        $permissionsByType = collect($documentTypes)->mapWithKeys(
            fn($type) => [$type["id"] => $type["permissions"]]
        );
        ...
        $filteredDocuments = collect($documents)
            ...
            ->map(function ($doc) use ($permissionsByType) {
                $doc["permissions"] = $permissionsByType[
                    $doc["document_type_id"]
                ] ?? [
                    "create" => false,
                    "view_all" => false,
    ...
    public function store(StoreFolderRequest $request)
    {
        try {
            ...
            $folder = Folder::create([
                ...
                "notify_allowed_user" =>
                    $validated["notify_allowed_user"] ?? null,
            ]);

            // 🔹 Création des liens dossier <-> départements
            if (
                !empty($validated["departmentIds"]) &&
                is_array($validated["departmentIds"])
            ) {
                $departmentFolders = collect($validated["departmentIds"])
                    ->map(function ($departmentId) use ($folder) {
                        return [
    ...
    public function getFoldersWithPermissionsForRoles(Request $request)
    {
        ...
        $department_folders = DepartmentFolder::with("folder")
            ->whereDepartmentId($departmentId)
            ->get();
    ...
    public function getAuthorizedDepartments(Request $request, $folderId)
    {
        try {
            ...
            $allowedDepartmentIds = DepartmentFolder::where(
                "folder_id",
                $targetFolderId
            )
                ->pluck("department_id")
            ...
            if (empty($allowedDepartmentIds)) {
                return response()->json(
                    [
                        ...
                        "departments" => [],
                        "message" =>
                            "Aucun département n’a accès à ce dossier.",
                    ],
                    200
                );
            }
            ...
            $departmentServiceUrl =
                config("services.department_service.base_url") . "/list-by-ids";

            $response = Http::withToken($request->bearerToken())->post(
                $departmentServiceUrl,
                [
            ...
            if ($response->failed()) {
                return response()->json(
                    [
                        "success" => false,
                        "message" =>
                            "Impossible de contacter le microservice des départements.",
                        "error" => $response->body(),
                    ],
                    500
                );
            ...
            return response()->json(
                [
                    "success" => false,
                    "message" =>
                        "Erreur lors de la récupération des départements autorisés.",
                    "error" => $th->getMessage(),
                ],
                500
            );
